...
mdep.c - Machine dependent parts of Andys Editor, Win32 version
...
AE_BOOLEAN unlink_file(const char *fn)
	...
	if ( stat(fn, &buf) != -1          &&      /* Can stat file       */
		...
		chmod(fn, buf.st_mode | S_IWRITE); /* Make it writable    */
...
if ( _stat(fn, &buf) != -1 )
	{
	if ( (buf.st_mode & _S_IFREG) == 0 ||	/* Not regular file */
	     (buf.st_mode & _S_IFCHR) != 0 )	/* Or is a device */
		{ fclose(fp); return NULL; }	/* Complain */
	}
}
/*...e*/
